-- Safe OTP functions migration - handles existing objects gracefully
-- Create OTP table for storing OTP codes (only if it doesn't exist)
...
BEGIN
    ...
    AND (otp_codes.expires_at < NOW() OR used = TRUE);
    
    -- Insert new OTP code
    INSERT INTO public.otp_codes (email, code, expires_at)
    VALUES (user_email, otp_code, otp_expires_at);
...
-- Grant execute permissions to service role (safe to run multiple times)
